...
target_image_name() {
	...
	if [ "$IMAGE_TYPE" = "live" ]; then
		if [ "$KALI_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Plague-Security-$KALI_VARIANT-$KALI_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Plague-Security-$KALI_VARIANT-$KALI_ARCH.$IMAGE_EXT"
		fi
	else
		if [ "$KALI_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Plague-Security-installer-$KALI_VARIANT-$KALI_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Plague-Security-installer-$KALI_VARIANT-$KALI_ARCH.$IMAGE_EXT"
		fi
	fi
...
--installer
--live
--variant <bugbounty / everything / kde / xfce>
...
case "$IMAGE_TYPE" in
	live)
		if [ ! -d "$(dirname $0)/plaguesec-config/variant-$KALI_VARIANT" ]; then
			echo "ERROR: Unknown variant of PlagueSec live configuration: $KALI_VARIANT" >&2
		fi
	...
	;;
	installer)
		if [ ! -d "$(dirname $0)/plaguesec-config/installer-$KALI_VARIANT" ]; then
			echo "ERROR: Unknown variant of PlagueSec installer configuration: $KALI_VARIANT" >&2
		fi
...
case "$IMAGE_TYPE" in
	...
	installer)
		...
		grep -v '^#' plaguesec-config/installer-$KALI_VARIANT/packages \
...
run_and_log echo -e "GENERATED PlagueSecurity IMAGE: $TARGET_DIR/$(target_image_name $KALI_ARCH)"
